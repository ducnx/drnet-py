...
def train(x):
    ...
    h_c1 = netEC(x_c1)
    h_c2 = netEC(x_c2)[0].detach() if opt.content_model[-4:] == 'unet' else netEC(
        x_c2).detach()  # used as target for sim loss
...
def main():
    for epoch in range(opt.niter):
        ...
        print('[%02d] rec loss: %.4f | sim loss: %.4f | scene disc acc: %.3f%% (%d)' % (
            epoch, epoch_rec_loss / opt.epoch_size, epoch_sim_loss / opt.epoch_size,
            100 * epoch_sd_acc / opt.epoch_size,
